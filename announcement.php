														...
														<div class="row">
									                        <div class="col-sm-12">
									                            ...
									                            <select data-placeholder="-SELECT-" class="form-control form-control-select2" name="teacherOption" data-fouc required>
																	...
																	<option value="<?= $row['TeacherID']; ?>"><?= $row['FirstName'].' '.$row['MiddleName'].' '.$row['LastName']; ?></option>
																	<?php } else { ?>
																	<option value=""></option>
									                                <?php
									                                	}
																		$teacherQuery = 'SELECT ID, FirstName, MiddleName, LastName FROM tbl_teacher WHERE isDeleted = 0 AND ID != ?';
																		$teacherStmt = $conn->prepare($teacherQuery);
																		$teacherStmt->execute([$row['TeacherID']]);
